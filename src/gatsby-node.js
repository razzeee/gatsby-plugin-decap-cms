...
exports.onPreInit = ({ reporter }) => {
  try {
    ...
    reporter.warn(
      `The netlify-cms package is deprecated, please install decap-cms-app instead. You can do this by running "npm install decap-cms-app"`
    )
  } catch (err) {
    // carry on
  }
  try {
    require.resolve(`netlify-cms-app`)
    reporter.warn(
      `The netlify-cms-app package is deprecated, please install decap-cms-app instead. You can do this by running "npm install decap-cms-app"`
    )
  } catch (err) {
    // carry on
  }
...
exports.onCreateWebpackConfig = (
  ...
  const config = {
    ...
    plugins: [
      // Remove plugins that either attempt to process the core Decap CMS
      ...
      /**
       * Provide a custom message for Decap CMS compilation success.
       */
      stage === `develop` &&
